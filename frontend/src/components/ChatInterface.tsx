...
function ChatInterface({ messages, isLoading, onSendMessage, messagesEndRef }: ChatInterfaceProps) {
  ...
  const renderMessages = (panelNumber: number, panelName: string, endRef?: React.RefObject<HTMLDivElement | null>) => (
    <>
      {messages.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-full text-gray-500 text-center p-4">
          <div className="text-5xl mb-4">💬</div>
          <h3 className="text-xl mb-2 text-gray-700 font-semibold">{panelName}</h3>
          ...
          {messages.map((message) => (
            ...
            >
              ...
              >
                ...
                <div className="leading-relaxed break-words whitespace-pre-wrap text-sm">{message.content}</div>
              </div>
            </div>
          ))}
          {isLoading && (
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <form className="sticky bottom-0 p-6 bg-white border-t-2 border-gray-300 shadow-lg shrink-0" onSubmit={handleSubmit}>
