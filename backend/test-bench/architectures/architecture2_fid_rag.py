...
class FiDRAGArchitecture(BaseRAGArchitecture):
    ...
    def query(self, query: str) -> ArchitectureResult:
        ...
        try:
            ...
            print(f"[FiD] Top 3 vector scores: {sorted(vector_scores.items(), key=lambda x: x[1], reverse=True)[:3]}")
            ...
            print(f"[FiD] BM25 scores: min={np.min(bm25_scores):.4f}, max={np.max(bm25_scores):.4f}, mean={np.mean(bm25_scores):.4f}")
            ...
            # Get top documents
            top_docs = sorted(combined_scores.items(), key=lambda x: x[1], reverse=True)[:10]
            print(f"[FiD] Top 10 combined scores: {[(doc_id, f'{score:.4f}') for doc_id, score in top_docs]}")
            ...
            if retrieved_docs:
                print(f"[FiD] Applying Fusion-in-Decoder on top {min(5, len(retrieved_docs))} documents...")
                # Encode query-document pairs
                query_doc_pairs = [f"{query} [SEP] {doc.content}" for doc in retrieved_docs[:5]]
                print(f"[FiD] Encoding {len(query_doc_pairs)} query-document pairs...")
                fused_embeddings = self.fusion_encoder.encode(query_doc_pairs, show_progress_bar=False)
                ...
                doc_embeddings = np.array([self.vector_embeddings[doc.doc_id] for doc in retrieved_docs[:5]])
                similarities = np.dot(doc_embeddings, fused_embedding) / (
                    np.linalg.norm(doc_embeddings, axis=1) * np.linalg.norm(fused_embedding) + 1e-8
                )
                ...
                print(f"[FiD] Selected {len(final_docs)} final documents for answer generation")
                print(f"[FiD] Document IDs: {[doc.doc_id for doc in final_docs]}")
                print(f"[FiD] Document previews: {[text[:100] + '...' for text in final_texts]}")
                ...
                print(f"[FiD] Metrics - Latency: {latency:.2f}s, Accuracy: {accuracy:.4f}, Confidence: {confidence_score:.4f}")
